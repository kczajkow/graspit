...
//
// $Id: world.h,v 1.41.2.2 2009/04/27 14:33:10 cmatei Exp $
//
//######################################################################
...
protected:
  ...
  //! This timer sensor is used to get input from devices such as Flock of Birds, Cyberglove etc
  SoTimerSensor *timerSensor;

  //! The length of the default dynamics time step
  double dynamicsTimeStep;  

  //! Structure for using the Cyber Glove
  CyberGlove *mGlove;

  //! Structure for using the Flock of Birds
  Flock *mFlock;

  //! Flag that shows if the cyberglove is currently in use
  bool mCyberGloveOn;

  //! Flag that shows if the Flock of Birds is currently in use
  bool mFlockOn;
  ...
  //! Callback routine for static environment for using external input and sensors
  static void timerCB(void *data, SoSensor *sensor);
  
  //! Processed information from the CyberGlove and sends it to appropriate robots
  void processCyberGlove();

  //! Processed information from the Flock and sends it to appropriate robots and bodies
  void processFlock();
...
public:	
  ...
  //! Turns on the CyberGlove processing
  void turnOnCyberGlove();
  //! Turns off the CyberGlove processing
  void turnOffCyberGlove();
  //! Returns the state of CyberGlove processing
  bool cyberGloveOn(){return mCyberGloveOn;}

  //! Turns on Flock of Birds processing
  void turnOnFlock();
  //! Turns off Flock of Bords processing
  void turnOffFlock();
  //! Sets which bird is the reference point for the others
  void setFlockBase(int masterBird = 1);
  //! Returns the current state of Flock of Birds processing
  bool flockOn(){return mFlockOn;}
  //! Returns the raw transform of one particulat bird in the Flock
  transf getBirdTran(int b = 1);
  //! Tells us which bird determines the position of the camera. 
  /*! If it set to zero, the camera is not run by a bird. If it is, that bird 
	  automatically becomes the master bird, the one relative to which all other 
	  bird positions are computed.
  */
  int mCameraBird;
  //! Keeps track of the camera transform when run by the flock of birds
