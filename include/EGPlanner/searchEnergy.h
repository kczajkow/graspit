...
//
// $Id: searchEnergy.h,v 1.25.2.2 2009/04/27 14:33:11 cmatei Exp $
//
//######################################################################
...
//! Computes the "quality" of a SearchState, which encapsulates the state of the hand 
/*!	This class can compute the "quality" (or in other terms "energy") of a
	SearchState. There are many ways of doing this which are all right now 
...
public:
	...
	/*! This is the main interface to this class. It is passed a SearchState* and returns whether the SearchState is 
		legal, and if so, it's energy. If noChange = true, it will re-set the world situation to what it was on entry.
		if not, it will leave the world in the state encapsulated in SearchState.
		if the SearchState is not legal, it will re-set the world before exiting regardless of the noChange flag.
	*/
	virtual void analyzeState(bool &isLegal, double &stateEnergy, const SearchState *state, bool noChange = true);
...
{
protected:
	const std::list<SearchState*> *mAvoidList;
	double mThreshold;
public:
	ClosureSearchEnergy() : SearchEnergy(), mAvoidList(NULL), mThreshold(0.3) {}

	void setThreshold(double t){mThreshold=t;}
	void setAvoidList(const std::list<SearchState*> *l){mAvoidList = l;}
	void analyzeState(bool &isLegal, double &stateEnergy, const SearchState *state, bool noChange = true);
};
