...
//
// $Id: egPlanner.h,v 1.25.2.2 2009/04/27 14:33:11 cmatei Exp $
//
//######################################################################
...
protected:
	...
	//! The current state of the planner (see the SearchState class for details)
	SearchState *mCurrentState;
	...
	const SearchState *mLastRenderState;
	...
	std::list<SearchState*> mBestList;
	//! Helper function that helps maintain a list of UNIQUE solutions.
	bool addToListOfUniqueSolutions(SearchState *s, std::list<SearchState*> *list, double distance);
	//! Helper function that returns a measure of how similar two states are.
	virtual double stateDistance(const SearchState *s1, const SearchState *s2);
...
public:
	...
	virtual const SearchState* getGrasp(int i);
	...
	SearchState *getTargetState(){return mTargetState;}
