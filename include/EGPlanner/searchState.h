...
//
// $Id: searchState.h,v 1.30.2.2 2009/04/27 14:33:11 cmatei Exp $
//
//######################################################################

#ifndef _searchstate_h_
#define _searchstate_h_
...
private:
	...
	double mValue;
public:
	SearchParameter(const char *name, double val=0) : mName(name), mValue(val) {}
...
/*!	A SearchState is the combination of a PositionState and a PostureState. 
	...
	grasp quality metrics. The "Search" part in "SearchState" is a legacy from 
	previous code; we might change this name at some point.

	Mainly, the SearchState contains two sets of variables: an instance of the 
	...
	The SearchState can be asked to render itself. It knows about what Hand it is 
	used on.
...
private:
	...
	//! Just stores a value that it's given; this class has no idea how to compute its own energy
	double mEnergy;
	//! Same here; somebody else needs to set this value
	bool mLegal;
	//! Stores the iteration number inside the search that this state was found at
	int mItNumber;
	//! Stores something that's very specific to the online planner; should be more elegantly coded
	double mDistance;
...
public:
	SearchState(Hand *h);
	SearchState(const SearchState *s){init();copyFrom(s);}
	~SearchState();
	inline void copyFrom(const SearchState *s);
	//! Resets both posture and position (currently sets all variables to 0)
	inline void reset();

	//! This SearchState becomes a snapshot of the CURRENT world state when the function is called.
	void saveCurrentHandState();

	void changeHand(Hand *h, bool sticky = false);
	Hand *getHand() const {return mHand;}
	void setObject(Body *b){mTargetObject = b;}
	Body *getObject() const {return mTargetObject;}
	...
	double distance(const SearchState *s) const;
	...
	static bool compareStates(const SearchState *s1, const SearchState *s2);
	//! Compares two states by their saved "distance" information
	static bool compareStatesDistances(const SearchState *s1, const SearchState *s2);
...
class TestSearchState : public SearchState
{
private:
	double mQuality;
	double mVolume;
...
public:
	TestSearchState(Hand* hand) : SearchState(hand) {}
	double getEpsilonQuality() const {return mQuality;} void setEpsilonQuality(double q){mQuality = q;}
	double getVolume() const {return mVolume;} void setVolume(double v){mVolume = v;}
...
void SearchState::copyFrom(const SearchState *s)
{
	mHand = s->mHand;
	...
	mEnergy = s->mEnergy;
	mLegal = s->mLegal;
	mItNumber = s->mItNumber;
	mDistance = s->mDistance;
...
*/
void createPositionSpaceSampling(const SearchState &seed, 
								 std::list<SearchState*> *sampling, int samples);
