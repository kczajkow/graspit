...
//
// $Id: graspTesterThread.h,v 1.9.2.2 2009/04/27 14:33:11 cmatei Exp $
//
//######################################################################
...
private:
	...
	std::list<SearchState*> mSolutionList;
	//! Stores candidates queued up for testing
	std::list<SearchState*> mCandidateList;
	...
	SearchState* popCandidate();
	//! Posts a new solution in the list
	void postSolution(SearchState*);

	//! The actual testing routine
	void testGrasp(SearchState *s);
...
public:
	...
	bool postCandidate(SearchState *s);
	//! Retrieve the least recently posted solution in the list
	SearchState* popSolution();
