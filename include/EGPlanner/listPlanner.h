...
//
// $Id: listPlanner.h,v 1.5.2.2 2009/04/27 14:33:11 cmatei Exp $
//
//######################################################################
...
{
protected:
	/*! The list of grasps to be tried by this planner. All SearchStates are deleted
		upon destruction of the planner */
	std::list<SearchState*> mInputList;

	//! The current position in the input list; shows the next grasp to be tried
	std::list<SearchState*>::iterator mPlanningIterator;
	...
	SearchState *getState(int index);
public:
	ListPlanner(Hand *h);
	~ListPlanner();
	...
	void setInput(std::list<SearchState*> input);
