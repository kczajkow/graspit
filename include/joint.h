...
//
// $Id: joint.h,v 1.16.2.2 2009/04/27 14:33:09 cmatei Exp $
//
//######################################################################
...
public:
 ...
 Joint(KinematicChain *k) : DOFnum(0), owner(k), jointNum(-1), draggerAttached(false), dynamicsVal(0.0),
  ...
  /*!
    ...
    configuration file.
  */
  virtual int initJoint(const char *info, int jnum) =0;
  ...
  //! Computes the 6x6 Jacobian of this joint wrt to a point in world coordinates
  void worldJacobian(transf toTarget, Matrix *J);

  //! Computes the 6x6 Jacobian of this joint wrt to a point in local point coordinates
  void localJacobian(transf toTarget, Matrix *J);
  ...
  //!iros09
  void setParameters(double couple, double k){mCouplingRatio = couple; mK =k;}
};
...
public:
  ...
  virtual int initJoint(const char *info, int jnum);
