...
//
// $Id: robot.h,v 1.46.2.2 2009/04/27 14:33:10 cmatei Exp $
//
//######################################################################
...
protected:
  ...
  //main load and specialized helper functions 
  //! The main load function that loads all the information from a QT stream
  int loadFromStream(QTextStream &stream, QString root);
  //! Looks for and loads approach direction from a QT stream
  int loadApproachDirection(QTextStream *stream);
  //! Looks for and loads Flock of Birds information from a given file
  int loadFlockData(QTextStream &stream);
 ...
 public:
  ...
  //! Load robot from a file. Almost always used to populate a robot class.
  ...
  //! Computes the dofvals that achieve a desired end pose for a chain. Not implemented yet.
  ...
  //! Calls the quasistatic computation method for contact, link and object forces
  virtual int accumulateQuasistaticForces(bool useDynamicDofForce);
  ...
  //iros09
  void setJointParameters(double couple, double k);
};
