...
//
// $Id: contact.cpp,v 1.47.2.1 2009/04/24 22:19:20 cmatei Exp $
//
//######################################################################
...
Contact::Contact(Body *b1, Body *b2, position pos, vec3 norm)
{
  ...
  updateCof();
  //this does not use our current contact hierarchy; should be updated
  if (cof == 0.0) {
    frictionType = FL;
    contactDim = 1;
    lmiDim = 1;
...
/*! Set up friction as a linearized circle; a PCWF can only have friction
	...
	force will be added later, the friction circle becomes a the more 
