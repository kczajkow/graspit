...
//
// $Id: graspitGUI.cpp,v 1.10.2.2 2009/04/27 14:33:13 cmatei Exp $
//
//######################################################################
...
//for database grasping project
#ifdef DBASE_PROJECT
#include "DBase/dbase_grasp.h"
#include "DBase/dbase_tester.h"
#endif
...
#ifdef DBASE_PROJECT
  //this is a special case for a project we are currently working on
  if (argc > 1 && ( !strcmp(argv[1],"dbase") || !strcmp(argv[1],"dbase_gripper") ) ) {
	  DBaseBatchPlanner *batchPlanner = new DBaseBatchPlanner(ivmgr,this);
	  if (!batchPlanner->processArguments(argc, argv)) {
		  return TERMINAL_FAILURE;
	  }
	  batchPlanner->startPlanner();
  }
  else if(argc > 1 && !strcmp(argv[1],"dbase_tester")){
	  DBaseTester dbtester;
	  if (!dbtester.processArgc(argc,argv)) {
		  return TERMINAL_FAILURE;
	  }
	  dbtester.executeBatch();
	  //we need Graspit to die now anyway
	  return TERMINAL_FAILURE;
  }
#endif
