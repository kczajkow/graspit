...
//
// $Id: dof.cpp,v 1.20.2.1 2009/04/24 22:19:20 cmatei Exp $
//
//######################################################################
...
/*! Reads the parameters (not the value) of this DOF from a stream. Used
	when reading robot configuration files, where we want to define a
	DOF, not set a particular value.
*/
bool
DOF::readParametersFromStream(QTextStream &stream)
{
	stream >> defaultVelocity >> maxForce >> Kp >> Kv >> draggerScale;	
...
bool
BreakAwayDOF::readParametersFromStream(QTextStream &stream)
{
	if (!DOF::readParametersFromStream(stream)) return false;
	stream >> mBreakAwayTorque;
	if (stream.status() != QTextStream::Ok || mBreakAwayTorque < 0) {
