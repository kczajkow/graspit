...
//
// $Id: searchEnergy.cpp,v 1.37.2.2 2009/04/27 14:33:15 cmatei Exp $
//
//######################################################################
...
void SearchEnergy::analyzeState(bool &isLegal, double &stateEnergy, const SearchState *state, bool noChange)
{
...
SearchEnergy::potentialQualityEnergy(bool verbose) const
{
	...
	double gq = -1 ,gv = -1;
	if (count >= 3) {
		mHand->getGrasp()->updateWrenchSpaces3D();
		gq = mEpsQual->evaluate3D();
		gv = mVolQual->evaluate3D();
	}
	if (verbose) {
...
SearchEnergy::autoGraspStep(int numCols, bool &stopRequest) const
{
	...
	int result = mHand->accumulateQuasistaticForces(false);
	if (result) {
		if (result > 0) {
...
SearchEnergy::compliantEnergy() const
{
	...
	int result = mHand->accumulateQuasistaticForces(true);
	if (result) {
		if (result > 0) {
...
void 
ClosureSearchEnergy::analyzeState(bool &isLegal, double &stateEnergy, const SearchState *state, bool noChange)
{
	//first we check if we are too close to any state in the avoid list
	if (mAvoidList) {
		std::list<SearchState*>::const_iterator it; int i=0;
