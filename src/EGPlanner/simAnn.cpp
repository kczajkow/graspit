...
//
// $Id: simAnn.cpp,v 1.29.2.2 2009/04/27 14:33:16 cmatei Exp $
//
//######################################################################
...
/*! The caller passes it a SearchState and a calculator that can be used
	to compute the quality (or in annealing terms "energy") of a SearchState. 
	This function computes the next state in the annealing schedule.

	See SimAnn::Result declaration for possible return values.
*/
SimAnn::Result 
SimAnn::iterate(SearchState *currentState, SearchEnergy *energyCalculator, SearchState *targetState)
{
...
SearchState* SimAnn::stateNeighbor(SearchState *s, double T, SearchState *t)
{
	SearchState *sn = new SearchState(s);
	if (t) {
