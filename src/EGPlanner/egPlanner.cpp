...
//
// $Id: egPlanner.cpp,v 1.38.2.1 2009/04/24 22:19:22 cmatei Exp $
//
//######################################################################
...
EGPlanner::getGrasp(int i)
{
	...
	std::list<SearchState*>::iterator it = mBestList.begin();
	for (int k=0; k<i; k++) {
		it++;
	}
	return (*it);
...
EGPlanner::showGrasp(int i)
{
	...
	const SearchState *s = getGrasp(i);
	s->execute();
	bool l; double e;
...
double
EGPlanner::stateDistance(const SearchState *s1, const SearchState *s2) {
...
*/
bool
EGPlanner::addToListOfUniqueSolutions(SearchState *s, std::list<SearchState*> *list, double distance)
{
