...
//
// $Id: searchState.cpp,v 1.35.2.2 2009/04/27 14:33:15 cmatei Exp $
//
//######################################################################
...
//----------------------------------------------------------------------------------
SearchState::SearchState(Hand *h)
{
	init();
...
bool
SearchState::compareStates(const SearchState *s1, const SearchState *s2)
{
...
bool
SearchState::compareStatesDistances(const SearchState *s1, const SearchState *s2)
{
	if (s1->getDistance() <= s2->getDistance()) return true; 
	return false;
}

void SearchState::changeHand(Hand *h, bool sticky)
{
	mHand = h;
...
void SearchState::saveCurrentHandState()
{
...
*/
void SearchState::setPositionType(StateType type, bool sticky)
{
...
/*! If the \a sticky flag is true, it will attempt to maintatin the same 
	...
	between two parameterization types.
*/
void SearchState::setPostureType(StateType type, bool sticky)
{
...
*/
void SearchState::setRefTran(transf t, bool sticky)
{
	transf totalTran;
	if (sticky) {
...
bool SearchState::readFromFile(FILE *fp)
{
	...
	float en,dist;
	if (fscanf(fp,"%f %d %f",&en, &mItNumber, &dist)<3) return false;
	mEnergy = en;
	mDistance = dist;
...
void SearchState::writeToFile(FILE *fp) const
{
	mPosture->writeToFile(fp); 
	mPosition->writeToFile(fp);
	fprintf(fp,"%f %d %f\n",mEnergy, mItNumber, mDistance);
}

bool SearchState::execute(Hand *h) const
{
	if (!h) h = mHand;
...
SearchVariable* SearchState::getVariable(int i)
{
	if ( i < 0 ) assert(0);
...
const SearchVariable* SearchState::getVariable(int i) const
{
...
SearchVariable* SearchState::getVariable (QString name)
{
...
const SearchVariable* SearchState::getConstVariable (QString name) const
{
...
double
SearchState::distance(const SearchState *s) const
{
	/*
...
void
SearchState::setIVMarkerColor(double r, double g, double b) const
{
	assert(IVMat);
...
void
SearchState::showVisualMarker()
{
	mHand->getWorld()->getIVRoot()->addChild( getIVRoot() );	
}

void
SearchState::hideVisualMarker()
{
	if (!IVRoot) return;
