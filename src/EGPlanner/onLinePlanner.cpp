...
//
// $Id: onLinePlanner.cpp,v 1.29.2.2 2009/04/27 14:33:15 cmatei Exp $
//
//######################################################################
...
OnLinePlanner::resetPlanner()
{
	...
	mCurrentBest = new SearchState(mCurrentState);
	return true;
}
...
double
OnLinePlanner::stateDistance(const SearchState *s1, const SearchState *s2)
{
...
OnLinePlanner::updateSolutionList()
{
	...
	mBestList.sort(SearchState::compareStatesDistances);
...
OnLinePlanner::mainLoop()
{
	...
	std::list<SearchState*>::iterator it = mCandidateList.begin();
	...
	if ( r == SimAnn::JUMP ) {
		...
		if (mCurrentState->getEnergy() < 0 || mCurrentState->getEnergy() < mCurrentBest->getEnergy()) {
			...
			if (!addToListOfUniqueSolutions(insertState, &mCandidateList,0.4)) {
				...
				mCandidateList.sort(SearchState::compareStates);
