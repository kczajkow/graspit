...
//
// $Id: listPlanner.cpp,v 1.6.2.2 2009/04/27 14:33:15 cmatei Exp $
//
//######################################################################
...
ListPlanner::resetParameters(){
	...
	mCurrentState = new SearchState(*mPlanningIterator);
}

void 
ListPlanner::setInput(std::list<SearchState*> input)
{
	if (isActive()) {
...
{
	std::list<SearchState*>::iterator it = mInputList.begin();
	int count = 0;
...
{
	SearchState *state = getState(index);
	if (!state) return;
...
ListPlanner::mainLoop()
{
	...
	if (legal) {
		...
		if (energy < worstEnergy) {
			SearchState *insertState = new SearchState(*mPlanningIterator);
...
{
	std::list<SearchState*>::iterator it;
...
void
ListPlanner::writeAggregates(std::ostream *out)
{
	double prevEn = -200;
	std::list<SearchState*>::iterator it;
	int i=0;
	...
	for (it = mBestList.begin(); it!=mBestList.end(); it++) {
		en = (*it)->getEnergy();
		if (en > -190.0 && prevEn < -190.0) {
			*out << " " << i;
		}
		if (en > -150.0 && prevEn < -150.0) {
			*out << " " << i;
		}
		if (en > -100.0 && prevEn < -100.0) {
			*out << " " << i;
		}
		if (en > 0) break;
		prevEn = en;
