...
//
// $Id: gws.cpp,v 1.20.2.2 2009/04/27 14:33:13 cmatei Exp $
//
//######################################################################
...
void minkowskiSum(Grasp *g,int c,int &wrenchNum,coordT *wrenchArray, Wrench currSum);
		   ...
		   std::vector<int> &hullIndices)
{
  ...
  for (i=0;i<numHyperPlanes;i++) {
    ...
    if (len>1e-11) {
	  ...
	  // we might have positive offsets if we are looking at non F-C grasps
...
/*! Given a set of hyperplanes that have already been computed and stored 
	internally in hyperPlaner, this will compute the metrics that we care
...
*/
int 
GWS::buildHyperplanesFromWrenches(void *wr, int numWrenches, int dimensions)
{
  ...
  FORALLfacets {
	  for (int d=0; d<dimensions; d++) {
		hyperPlanes[i][d] = facet->normal[d];
	  }
	  for (int d = dimensions; d<6; d++) {
		  hyperPlanes[i][d] = 0;
	  }
...
/*! Builds an L1 GWS by simply taking the union of all the contact wrenches,
	and using these as the input for the convex hull operation (performed by 
	qhull). The output of qhull is the list of equations of the hyperplanes 
	that bound the hull as well as the hull area and hull volume. These are 
	all saved within the GWS.
...
L1GWS::build()
{
  ...
  DBGP(" in gws build");
  if (useGravity) {
	  DBGP("Gravity: " << gravDirection.x() << " " << gravDirection.y() << " " << gravDirection.z());
  }
  ...
  for (i=0;i<grasp->getNumContacts();i++) {
	  wrenchCount += grasp->getContact(i)->getMate()->numFCWrenches;
  }
  ...
  for (i=0;i<grasp->getNumContacts();i++) {    
    for (w=0; w<grasp->getContact(i)->getMate()->numFCWrenches; w++) {
      ...
      *p++ = grasp->getContact(i)->getMate()->wrench[w].force.z();
	  ...
	  if (useGravity) {
		  *(p-3) = *(p-3) + gravDirection.x();
		  *(p-2) = *(p-2) + gravDirection.y();
		  *(p-1) = *(p-1) + gravDirection.z();
	  }
  ...
  int result;
  try {
	result = buildHyperplanesFromWrenches(array,wrenchCount);
  } catch(...) {
...
L1GWS::build3D()
{
  ...
  coordT *array = new coordT[wrenchCount*3];
  if (!array) std::cerr << "array is NULL!! in L1GWS::Build(). wrenchCount: " << 
		wrenchCount << std::endl;
  coordT *p = &array[0];
  ...
  int result;
  try {
	result = buildHyperplanesFromWrenches(array,wrenchCount,3);
  } catch(...) {
...
minkowskiSum(Grasp *g,int c,int &wrenchNum,coordT *wrenchArray,Wrench prevSum)
{
  ...
  for (int w=0; w<g->getContact(c)->getMate()->numFCWrenches; w++) {
...
LInfGWS::build()
{
  ...
  double numWrenchTest = 1;
  for (i=0;i<grasp->getNumContacts();i++) {
	  numWrenchTest *= grasp->getContact(i)->getMate()->numFCWrenches + 1;
  }
  //double numWrenchTest = pow(Contact::numFCWrenches+1, grasp->getNumContacts());
