...
//
// $Id: ivmgr.cpp,v 1.52.2.2 2009/04/27 14:33:13 cmatei Exp $
//
//######################################################################
...
IVmgr::drawDynamicForces()
{
  ...
  if (!world->dynamicsAreOn()) {
    ...
    for (b=0;b<world->getNumGB();b++) {
      ...
      numContacts += sz;
	  numChildren = world->getGB(b)->getIVContactIndicators()->getNumChildren();
	  for (i=numChildren-1; i>=sz; i--){
    ...
    if (contactForceBlinkerVec) delete (contactForceBlinkerVec);
    contactForceBlinkerVec = (SoBlinker **)malloc(numContacts*sizeof(SoBlinker *));
  }
  ...
  for (b=0; b<world->getNumGB(); b++) {
    ...
    for (cp=contactList.begin(),i=0;cp!=contactList.end();cp++,i++,contactNum++) {
      contactForce = (*cp)->getDynamicContactWrench();

	  DBGP("GraspableBody " << b << ", Contact " << i << ": " <<
		   contactForce[0] << " " << contactForce[1] << " " << contactForce[2]);
      
      forceVec.setValue(contactForce[0],contactForce[1],contactForce[2]);
      //      if (forceVec.length() == 0.0) continue;
      ...
      if (world->dynamicsAreOn()) {
		...
		contactForceBlinkerVec[contactNum] = new SoBlinker;
		contactForceBlinkerVec[contactNum]->addChild(ptr);
		contactForceBlinkerVec[contactNum]->on = false;
		contactForceBlinkerVec[contactNum]->whichChild = 0;
		world->getGB(b)->getIVContactIndicators()->insertChild(contactForceBlinkerVec[contactNum],lastChild);
      }
    }
  }
}
...
IVmgr::transRot(DraggerInfo *dInfo)
{
  ...
  if (!translating && myCenterball->center.getValue() != dInfo->lastCent) {
    ...
    //dInfo->centerballTransl = recenterTran.translation().toSbVec3f();
  ...
  */
  ...
  if (translating) {
    if ((myHandleBox->translation.getValue() - dInfo->lastTran).length()<
	0.00001) {
   ...
   // std::cout << "desired Translation: " << desiredTranslation << std::endl;
    //    //not needed?
...
#ifdef GRASPITDBG    
    std::cout << "new pos: "<<dInfo->selectedElement->getTran() << std::endl;
#endif
  if (translating) {
    ...
    SbRotation newRot =
      dInfo->selectedElement->getTran().rotation().toSbRotation();
  ...
  if (enabled)
    dInfo->dragger->enableValueChangedCallbacks(TRUE);
}
...
IVmgr::makeCenterball(WorldElement *selectedElement,Body *surroundMe)
{
  ...
  float maxRad = (bba->getBoundingBox().getMax() -
		  bba->getBoundingBox().getMin()).length();

  delete bba;

  maxRad /= 2.0;
  
  draggerTran->translation = 
    selectedElement->getTran().translation().toSbVec3f();
  draggerTran->scaleFactor.setValue(maxRad,maxRad,maxRad);

  myCenterball->rotation = 
    selectedElement->getTran().rotation().toSbRotation();
 ...
 // sep->ref();
  ...
  dInfo->lastCent = myCenterball->center.getValue();
#ifdef GRASPITDBG
  std::cout << "lastCent " << dInfo->lastCent[0] << " "<<dInfo->lastCent[1]<<" "<<dInfo->lastCent[2]<<std::endl;
#endif
...
#ifdef GRASPITDBG
    std::cout <<"0 volume bounding box!"<<std::endl;
#endif
    return;
  }
...
#ifdef GRASPITDBG
	  std::cout << "removing dragger info\n";
#endif
	}
	else dp++;
      }
