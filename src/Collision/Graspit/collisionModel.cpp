...
//
// $Id: collisionModel.cpp,v 1.6.2.1 2009/04/24 22:19:22 cmatei Exp $
//
//######################################################################
...
double Leaf::getMedianProjection(const vec3 &axis)
{
	...
	nth_element(projections.begin(), projections.begin() + projections.size()/2, projections.end());
...
{	
	...
	if (mBbox.halfSize.y() > mBbox.halfSize.x()) {
		DBGA("Unexpected bonding box dominant axis. Extents: ");
