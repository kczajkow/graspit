...
MainWindow::MainWindow(QWidget *parent) 
{
	...
	QObject::connect(mUI->graspDBase_GUIAction, SIGNAL(triggered()), 
					 this, SLOT(graspDBase_GUIAction_activated()));
	QObject::connect(mUI->graspCompliantPlannerAction, SIGNAL(triggered()), 
					 this, SLOT(graspCompliantPlanner()));
	// -- sensors menu
	QObject::connect(mUI->sensorsCyberGloveOnAction, SIGNAL(triggered()), 
					 this, SLOT(cyberGloveOn()));
	QObject::connect(mUI->sensorsCyberGloveOffAction, SIGNAL(triggered()),
					 this, SLOT(cyberGloveOff()));
	QObject::connect(mUI->sensorsCyberGloveCalibrateAction, SIGNAL(triggered()),
					 this, SLOT(cyberGloveCalibrate()));
	QObject::connect(mUI->sensorsFlock_of_BirdsOnAction, SIGNAL(triggered()),
					 this, SLOT(flockOn()));
	QObject::connect(mUI->sensorsFlock_of_BirdsOffAction, SIGNAL(triggered()),
					 this, SLOT(flockOff()));
	QObject::connect(mUI->sensorsBarrett_HandAction, SIGNAL(triggered()),
					 this, SLOT(sensorsBarrettHandAction()));
	QObject::connect(mUI->sensorsGrasp_CaptureAction, SIGNAL(triggered()),
	...
	// -- ROS menu
	QObject::connect(mUI->rosInit_ROSAction, SIGNAL(triggered()), this, SLOT(rosInit_ROSAction_activated()));
	QObject::connect(mUI->rosShutdown_ROSAction, SIGNAL(triggered()), this, SLOT(rosShutdown_ROSAction_activated()));
	QObject::connect(mUI->rosScansAction, SIGNAL(triggered()), this, SLOT(rosScansAction_activated()));
	QObject::connect(mUI->rosEigengridsAction, SIGNAL(triggered()), this, SLOT(rosEigengridsAction_activated()));
	// -- contacts
...
void MainWindow::destroy()
{
	...
	if (getROSState() == ROS_READY) {
		rosShutdown_ROSAction_activated();
	}
#endif
...
#ifdef ROS_GRASPIT
  rosInit_ROSAction->setEnabled(TRUE);
  rosScansAction->setEnabled(TRUE);
  rosShutdown_ROSAction->setEnabled(FALSE);
#else
  mUI->rosInit_ROSAction->setEnabled(FALSE);
  mUI->rosScansAction->setEnabled(FALSE);
  mUI->rosShutdown_ROSAction->setEnabled(FALSE);
#endif
...
/*!
  ...
  extension, the ".wld" extension is added.
*/
...
void MainWindow::fileImportRobot()
{
	...
	QString fn( QFileDialog::getOpenFileName(mWindow, QString(), dir, "GraspIt Robot Files (*.cfg)") );
	if ( !fn.isEmpty() ) {
...
{
#ifdef COIN2
  QString fn( QFileDialog::getOpenFileName(mWindow, QString(), QString(getenv("GRASPIT"))+QString("/models/obstacles"),
   "Inventor Files (*.iv*);;VRML Files (*.wrl)" ) );
#else  
  QString fn( QFileDialog::getOpenFileName(mWindow, QString(), QString(getenv("GRASPIT"))+QString("/models/obstacles"),
   "Inventor Files (*.iv*)" ) );
#endif
...
{
#ifdef COIN2
  QString fn( QFileDialog::getOpenFileName(mWindow, QString(),QString(getenv("GRASPIT"))+QString("/models/objects"),
   "Inventor Files (*.iv*);;VRML Files (*.wrl)" ) );
#else  
  QString fn( QFileDialog::getOpenFileName(mWindow, QString(),QString(getenv("GRASPIT"))+QString("/models/objects"),
   "Inventor Files (*.iv*)" ) );
...
void MainWindow::updateGraspMenu()
{
	...
	mUI->graspContact_ExaminerAction->setEnabled(handFound);
	mUI->graspEigenGrasp_PlannerAction->setEnabled(handFound);
    mUI->graspCompliantPlannerAction->setEnabled(handFound);
#ifdef DBASE_PROJECT
	mUI->graspDBase_GUIAction->setEnabled(handFound);
#else
	mUI->graspDBase_GUIAction->setEnabled(false);
#endif
	mUI->sensorsCyberGloveCalibrateAction->setEnabled(handFound);
	mUI->sensorsGrasp_CaptureAction->setEnabled(handFound);
...
*/
void MainWindow::graspCreateProjection()
{
	GWSProjDlg *dlg = new GWSProjDlg(mWindow);
	if ( dlg->exec() == QDialog::Accepted ) {  
		Grasp *grasp = world->getCurrentHand()->getGrasp();
...
void MainWindow::graspDBase_GUIAction_activated()
{
#ifdef DBASE_PROJECT
...
void MainWindow::cyberGloveOn()
{
	world->turnOnCyberGlove();
	DBGA("CyberGlove turned ON");
}

void MainWindow::cyberGloveOff()
{
	world->turnOffCyberGlove();
	DBGA("CyberGlove turned OFF");
}

void MainWindow::flockOn()
{
	world->turnOnFlock();
	DBGA("Flock of Birds turned ON");
}


void MainWindow::flockOff()
{
	world->turnOffFlock();
	DBGA("Flock of Birds turned OFF");
}

void MainWindow::cyberGloveCalibrate()
{ 
	bool collisions = !world->collisionsAreOff();
	if (collisions) {
		world->toggleAllCollisions(false);
	}

	GloveCalibrationDlg *dlg = new GloveCalibrationDlg(mWindow);
	dlg->setAttribute(Qt::WA_ShowModal, true);
	dlg->exec();

	delete dlg;
	if (collisions) {
		world->toggleAllCollisions(true);
	}
}

void MainWindow::graspCapture()
{
	GraspCaptureDlg *dlg = new GraspCaptureDlg(mWindow);
	dlg->setAttribute(Qt::WA_ShowModal, false);
	dlg->setAttribute(Qt::WA_DeleteOnClose, true);
	dlg->setWorld(world);
...
// -------------------------------------- ROS menu ------------------------------------------------
void MainWindow::rosScansAction_activated()
{
#ifdef ROS_GRASPIT
	ScanDlg *dlg = new ScanDlg(mWindow);
	dlg->setAttribute(Qt::WA_ShowModal, false);
	dlg->setAttribute(Qt::WA_DeleteOnClose, true);
	dlg->show();	
#endif
...
void MainWindow::rosInit_ROSAction_activated()
{
#ifdef ROS_GRASPIT
	initROS();
	if ( getROSState() == ROS_READY ) {
		rosInit_ROSAction->setEnabled(FALSE);
		rosShutdown_ROSAction->setEnabled(TRUE);
	}
#endif
}

void MainWindow::rosShutdown_ROSAction_activated()
{
#ifdef ROS_GRASPIT
	shutdownROS();
	rosInit_ROSAction->setEnabled(FALSE);
...
void MainWindow::rosEigengridsAction_activated()
{
#ifdef ROS_GRASPIT
