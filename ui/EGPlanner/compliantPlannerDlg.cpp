...
//
// $Id: compliantPlannerDlg.cpp,v 1.12.2.1 2009/04/24 22:19:25 cmatei Exp $
//
//######################################################################
...
void 
CompliantPlannerDlg::addCartesianSamples(const SearchState &seed, 
										 std::list<SearchState*> *sampling, 
										 int samples, double x, double y, double z)
{
	...
	for (int m=0; m<samples; m++) {
		...
		SearchState *newState = new SearchState(&seed);
...
void 
CompliantPlannerDlg::gridEllipsoidSampling(const SearchState &seed,
										   std::list<SearchState*> *sampling, 
										   int samples)
{
	double a = seed.readPosition()->getParameter("a");
	...
	while (m1 < 2*sz1){
		...
		while (m2 < 2*sz2) {
			...
			for(int rot=0; rot < rotSamples; rot++) {
				...
				tr = rotTran * tr;
				SearchState* seed = new SearchState(mHand);
				seed->setObject(mObject);
				seed->setRefTran(mObject->getTran(), false);
				seed->setPostureType(POSE_DOF, false);
...
CompliantPlannerDlg::showResult()
{
	...
	if ( d!=0 ){
		const SearchState *s = mPlanner->getGrasp(mBestGraspNum);
...
CompliantPlannerDlg::designTestButtonClicked()
{
	...
	mTR = mTFrom;
	mSR = mSFrom;
	mHand->setJointParameters(1.0 * mTR, 1600 * 1.0e6 * mSR);
	mPlanner->setEnergyType(ENERGY_COMPLIANT);
